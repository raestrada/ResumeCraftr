...
def get_vector_store_id_by_name(agent_name: str) -> str:
    ...
    console.print(f"[bold red]No vector store found with name '{expected_name}'.[/bold red]")
    return None
...
def upload_files_to_vector_store(vector_store_id: str, progress: Progress = None, task=None):
    ...
    console.print(f"[bold green]Files uploaded successfully to vector store '{vector_store_id}'.[/bold green]")
...
def create_or_get_agent():
    ...
    console.print(f"[bold yellow]Agent '{agent_name}' not exists, creating.[/bold yellow]")
    ...
    console.print(f"[bold green]Agent '{agent_name}' created successfully.[/bold green]")
    return assistant
